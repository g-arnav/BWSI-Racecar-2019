...
#source is camera feed and image is the comparison image
...
def sift_det(image,source):
    ...
    # Reads in the image for comparison image
    img1 = cv2.imread(image, 0)    
    #frame = source
    #frame = cv2.imread(source, 0)        
    ...
    """ if frame is None: 
        print("no image")            # Did we get an image at all?
        return  """
    ################################################### Shape Computation
    ...
    upper = np.array([180, 255, 55], dtype = "uint8")     
