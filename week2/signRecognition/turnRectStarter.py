...
def sift_det(image):
    ...
    while True:
        ...
        contours = sorted(contours, key = "YOUR METRIC HERE", reverse = True)[1:]   # Removes contouring of display window

        if len(contours) != 0:
            # TODO:  
            # Draws the max of the contours arrays with respect to the "key" chosen above
            contours_max = max(contours, key = "YOUR METRIC HERE")
            ...
            if w*h > "(int) Area lower limit here":
